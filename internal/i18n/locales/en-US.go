...
var MessagesEnUS = map[string]string{
	...
	"config.enable_sticky_key":                 "Enable Sticky Key Mode",
	"config.enable_sticky_key_desc":            "Keep using the current key until it reaches the consecutive failure threshold, then cool down, blacklist it, and switch to a random next key.",
	"config.sticky_key_failure_threshold":      "Sticky Key Failure Threshold",
	"config.sticky_key_failure_threshold_desc": "Consecutive failures before the sticky key enters global cooldown.",
	"config.sticky_key_cooldown_seconds":       "Sticky Key Cooldown (seconds)",
	"config.sticky_key_cooldown_seconds_desc":  "How long to reject all requests after the sticky key reaches the failure threshold.",
