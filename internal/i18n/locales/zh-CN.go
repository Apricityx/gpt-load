...
var MessagesZhCN = map[string]string{
	...
	"config.enable_sticky_key":                 "启用粘性 Key 模式",
	"config.enable_sticky_key_desc":            "启用后持续使用当前 Key，直到连续失败达到阈值后进入冷却，再拉黑并随机切换到下一个 Key。",
	"config.sticky_key_failure_threshold":      "粘性 Key 失败阈值",
	"config.sticky_key_failure_threshold_desc": "粘性 Key 连续失败多少次后触发全局冷却。",
	"config.sticky_key_cooldown_seconds":       "粘性 Key 冷却时间（秒）",
	"config.sticky_key_cooldown_seconds_desc":  "粘性 Key 达到失败阈值后拒绝所有请求的时间。",
