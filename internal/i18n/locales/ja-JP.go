...
var MessagesJaJP = map[string]string{
	...
	"config.enable_sticky_key":                 "スティッキーキーを有効化",
	"config.enable_sticky_key_desc":            "現在のキーを連続失敗しきい値に達するまで使い続け、クールダウン後にブラックリスト化してランダムな次のキーへ切り替えます。",
	"config.sticky_key_failure_threshold":      "スティッキーキー失敗しきい値",
	"config.sticky_key_failure_threshold_desc": "グローバルクールダウンを開始するまでの連続失敗回数。",
	"config.sticky_key_cooldown_seconds":       "スティッキーキークールダウン（秒）",
	"config.sticky_key_cooldown_seconds_desc":  "失敗しきい値到達後に全リクエストを拒否する時間。",
